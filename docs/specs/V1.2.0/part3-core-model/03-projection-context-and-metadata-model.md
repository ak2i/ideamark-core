# 03 — Projection Context and Metadata Model
...
This section defines how Projection Context and document metadata are treated in the IdeaMark Core Model.
...
## 3.2 Projection Context Definition

A Projection Context is the reuse strategy, interpretive constraint, or generation context that shapes how Original Source material is decomposed into an IdeaMark document.
...
Projection Context is not final meaning.
...
## 3.3 Projection Context in Part 3

Part 3 treats Projection Context as an input to Decomposition and as metadata needed for later reconstruction.
...
The same Original Source may generate different IdeaMark documents under different Projection Contexts.
...
Projection Context determines which local source windows become Sections, how reusable material is shaped into Entities, and what role placements become Occurrences.

## 3.4 Projection Context Is Not Projection Governance
...
Part 3 only requires that the Projection Context relevant to generation can be recorded or referenced sufficiently for later use.
...
4. **Projection traceability** — record the Projection Context or references that shaped Decomposition.
...
- the document is generated from a composite Projection Context;
...
The Core Model should allow systems to record enough context to understand which source material and Projection Context shaped a given structure.
...
later user or tool
  ...
  -> identify generation Projection Context
...
Projection Context and metadata should preserve the following invariants:

1. Decomposition is Projection-guided, not general extraction.
2. Projection Context shapes Sections, Occurrences, and Entities.
...
Projection Context explains why an IdeaMark document has the structure it has.
