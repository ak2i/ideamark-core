---
ideamark_version: 1
doc_id: "imdoc.breakdown.sec-kng-hakone-01"
parent_doc: "imdoc.sample.policy_compare.fhi-5cases"
section_id: "SEC-KNG-HAKONE-01"
generated_by: "breakdown-sim"
created_at: "2026-01-11"
---

# Breakdown: SEC-KNG-HAKONE-01

section_id: "SEC-KNG-HAKONE-01"
anchorage: ["place:hakone", "problem-framing"]
entities:
  - id: "IE-KNG-HAKONE-PROB-01"
    kind: "problem"
    atomic_state: true
    content: "交通混雑（道路・駐車・公共交通のピーク集中）が体験価値と生活圧を同時に悪化させる。"
  - id: "IE-KNG-HAKONE-PROB-02"
    kind: "problem"
    atomic_state: true
    content: "受け皿（シャトル/駐車/動線）の運用が“局所最適”になりやすく、全体の分散設計が必要。"
relations:
  - type: "generalizes"
    from: "IE-UK-LD-OVR-01"
    to: "IE-KNG-HAKONE-PROB-01"

## 13.2 Section: 箱根の介入候補（anchorage: place.hakone.measures）



---
# Expansion (conservative): SEC-KNG-HAKONE-01
mode: "conservative"

## Section: 箱根の課題仮説（拡張：slot-based）
section_id: "SEC-KNG-HAKONE-01"
anchorage: ["place:hakone", "problem-framing", "conservative-expansion", "slots"]

entities:
  - id: "IE-KNG-HAKONE-PROB-SLOT-01"
    kind: "projection"
    atomic_state: true
    content: "混雑発生源スロット（道路区間/駅/駐車場/時間帯）"
  - id: "IE-KNG-HAKONE-PROB-SLOT-02"
    kind: "projection"
    atomic_state: true
    content: "住民影響スロット（騒音/通行阻害/生活動線）"
  - id: "IE-KNG-HAKONE-PROB-SLOT-03"
    kind: "projection"
    atomic_state: true
    content: "交通事業者調整スロット（ダイヤ/料金/優先度）"
