...
from models.pattern_models import PatternResponse, PatternValidationResponse, RefGenerationResponse, PatternMergeResponse, PatternSearchResponse
import sys
import os
...
async def generate_ref(
    ...
    try:
        ...
        ref_data = {
            "ref": {
                ...
                "access": {
                    ...
                    "visibility": pattern_data.get('access', {}).get('visibility', 'public')
                },
                "metadata": {
        ...
        if ref_errors:
            raise HTTPException(status_code=400, detail=f"Generated ref validation failed: {ref_errors}")
...
async def merge_patterns(
    request: PatternMergeRequest,
    user: Dict[str, Any] = Depends(get_current_user)
):
    try:
        logger.info(f"Merging patterns {request.pattern_ids} for user {user.get('sub', 'unknown')}")
        
        if len(request.pattern_ids) < 2:
            raise HTTPException(status_code=400, detail="At least 2 patterns required for merge")
        
        ref_paths = []
        for pattern_id in request.pattern_ids:
            ref_path = storage.get_ref_path(pattern_id)
            if not os.path.exists(ref_path):
                raise HTTPException(status_code=404, detail=f"Reference file not found for pattern {pattern_id}")
...
async def search_patterns(
    ...
    try:
        logger.info(f"Searching patterns with query '{query}' for user {user.get('sub', 'unknown')}")
        ...
        for ref_data in all_refs:
            ...
            if pattern_type and ref.get('type') != pattern_type:
                continue
        ...
        matching_refs = matching_refs[:limit]
