...
def validate_pattern(pattern_data: Dict[str, Any]) -> List[str]:
    ...
    if 'type' in pattern_data and pattern_data['type'] not in ['design', 'architectural', 'behavioral', 'creational']:
...
async def generate_ref(
    ...
    errors = validate_pattern(pattern_data)
    if errors:
        raise HTTPException(status_code=400, detail=f"Pattern validation failed: {errors}")
...
async def merge_patterns(
    ...
    merged_pattern = {
        ...
        "problem": {
            "summary": "Merged pattern combining multiple solutions"
        },
        "solution": {
            "approach": "Combined approach from multiple patterns"
        },
        "access": {
