...
def test_mcp_server():
    ...
    try:
        response = requests.get(
            f"{base_url}/v1/pattern/search?query=test&limit=5",
            headers=headers
        )
    ...
    test_pattern_2 = {
        ...
        "problem": {
            "summary": "Second test problem"
        },
        "solution": {
            "approach": "Second test solution"
        },
        "access": {
            "uri": "file://test2.yaml",
            "visibility": "public"
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"  Patterns merged: {merge_data['merge_summary']['patterns_merged']}")
        else:
            print(f"  Error: {response.text}")
    except Exception as e:
